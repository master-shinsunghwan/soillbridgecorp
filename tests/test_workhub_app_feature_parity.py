...
class WorkhubAppFeatureParityTests(unittest.TestCase):
    ...
    def test_cs_request_from_ledger_menu_supports_mail_attachments(self) -> None:
        ...
        self.assertIn('id="csAttachmentList"', html_source)
        self.assertIn("cs-attachment-remove", html_source)
        self.assertIn("function removeCsAttachment", html_source)
        self.assertIn("new DataTransfer()", html_source)
        self.assertIn('data-cs-attachment-remove="${index}"', html_source)
        ...
        self.assertIn('csAttachmentList?.addEventListener("click"', html_source)
    ...
    def test_ledger_cs_popup_and_cell_edit_activation_are_workspace_safe(self) -> None:
        ...
        self.assertIn("function scrollEditableCellIntoSheetView(cell)", html_source)
        self.assertIn('const wrap = cell?.closest(".ledger-wrap");', html_source)
        self.assertIn("wrap.scrollLeft +=", html_source)
        self.assertIn("wrap.scrollTop +=", html_source)
        self.assertIn("nextCell.focus({ preventScroll: true });", html_source)
        self.assertIn("scrollEditableCellIntoSheetView(nextCell);", html_source)
        self.assertNotIn('nextCell.focus({ preventScroll: false });', html_source)
    ...
    def test_desktop_launchers_register_user_startup_on_first_run(self) -> None:
        ...
        self.assertIn("def start_desktop_update_check()", vps_launcher)
        self.assertIn("def launch_pending_desktop_update()", vps_launcher)
        self.assertIn("start_desktop_update_check()", vps_launcher)
        self.assertIn("launch_pending_desktop_update()", vps_launcher)
        self.assertIn("New-WorkhubDesktopShortcut", desktop_installer)
        self.assertNotIn("SoilbridgeWorkhubDesktop_AutoStart.vbs", desktop_installer)
    ...
    def test_excel_downloads_keep_object_url_until_browser_starts_download(self) -> None:
        for app_file in (
            ...
            if "_workhub_zip_inspect" not in app_file.parts:
                ...
                self.assertIn('id="desktopDownloadSettings"', html_source)
                self.assertIn('id="desktopDownloadChoose"', html_source)
                self.assertIn('id="desktopDownloadReset"', html_source)
                self.assertIn("function desktopDownloadSettingsBridgeApi()", html_source)
