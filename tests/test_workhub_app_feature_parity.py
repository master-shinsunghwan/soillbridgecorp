...
class WorkhubAppFeatureParityTests(unittest.TestCase):
    ...
    def test_distribution_mail_tree_only_opens_writable_mail_flows(self) -> None:
        for app_file in (
            ...
            self.assertIn('id="stockVendorContactSelect"', html_source)
            self.assertIn('id="stockRecipientEmailInput"', html_source)
            ...
            self.assertIn('stockVendorContactSelect?.addEventListener("change", applySelectedStockVendor)', html_source)
