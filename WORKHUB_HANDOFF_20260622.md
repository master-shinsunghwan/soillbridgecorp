...
이 전달용 폴더는 (주)소일브릿지 업무자동화 프로그램의 현재 작업본을 다른 PC에서 검토하고 이어서 개발할 수 있도록 만든 로컬 실행용 패키지입니다.
...
1. `soillbridgecorp_다른PC_실행용_폴더`를 다른 PC 원하는 위치에 폴더째 복사합니다.
2. 폴더 안의 `RUN_WORKHUB.cmd`를 더블클릭합니다.
3. 브라우저가 열리면 `http://127.0.0.1:8770/` 또는 `/login`으로 접속합니다.
4. `admin / admin1234`로 로그인합니다.

전달용 폴더에는 `runtime/python`이 포함되어 있어 Python을 따로 설치하지 않아도 실행할 수 있습니다. 큰 런타임 폴더가 들어 있으므로 ZIP으로 다시 압축하지 말고 폴더째 복사하는 것을 권장합니다.

실행이 실패하면 압축을 푼 폴더의 `workhub_run_error.log`를 확인합니다.
...
- 전달용 폴더를 ZIP으로 다시 압축하면 Python 실행 파일 일부가 누락될 수 있습니다. 가능하면 폴더째 복사하세요.
