...
포터블 압축본에는 `runtime/python`이 포함되어 있어 Python을 따로 설치하지 않아도 실행할 수 있습니다. 일반 압축본으로 전달받은 경우에는 Python 3.11 이상이 필요하며, 필요 패키지가 없으면 실행 스크립트가 `requirements.txt` 기준으로 설치를 시도합니다.
...
- 압축 파일은 반드시 먼저 풀고 실행해야 합니다. ZIP 내부에서 바로 `cmd`를 실행하면 정상 동작하지 않습니다.
