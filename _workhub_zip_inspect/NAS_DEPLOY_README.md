...
CRM 메신저 연동을 카카오 챗봇/외부 웹훅과 연결할 때는 아래 값을 추가로 고정할 수 있습니다.

```yaml
WORKHUB_PUBLIC_BASE_URL: "https://업무도메인.example.com"
WORKHUB_CRM_WEBHOOK_TOKEN: "soilbridge-crm-webhook-private-token"
```

이 값을 비워두면 앱이 `config/crm_webhook_token.txt`에 토큰을 자동 생성합니다.  
`WORKHUB_PUBLIC_BASE_URL`은 CRM 메신저 설정 화면에 표시할 전체 HTTPS 엔드포인트입니다.  
카카오 챗봇 스킬 서버에서는 `https://도메인/api/crm-messenger-webhook` URL과 `X-Workhub-Webhook-Token` 헤더 값을 등록해야 합니다.  
`WORKHUB_CRM_WEBHOOK_TOKEN`을 환경변수로 고정하면 화면에서 토큰을 재발급할 수 없습니다.
